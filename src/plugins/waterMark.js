...
let setWatermark = (
  ...
  cans.font = fontSize + "px Vedana"; // 字体大小
  cans.fillStyle = fontColor; // 水印的颜色
...
// 该方法只允许调用一次
watermark.set = (str, str1, fontColor, fontSize) => {
  let id = setWatermark(str, str1, fontColor, fontSize);
  setInterval(() => {
    if (document.getElementById(id) === null) {
      id = setWatermark(str, str1, fontColor, fontSize);
    }
  }, 500);
  window.onresize = () => {
    setWatermark(str, str1, fontColor, fontSize);
  };
};
